...
Barkley STANDS UPRIGHT on his hind legs at the right of the panels, roughly as tall as the tallest panel, one front paw raised flat and pressed against the side of the nearest panel as if tallying up the score, the other paw planted on his hip with the elbow bent out.""",
    "ichiban-ticket-reveal": """an Ichiban Kuji prize-ticket reveal. This must read as drawing and revealing one physical lottery ticket, not as a capsule machine, receipt printer, slot machine, or spinning-wheel game.
On the left, ONE thick deep-blue #02569B Ichiban Kuji ticket box stands on the paper. It is a simple upright box with a wide top opening and a small stack of plain gold #F8B003 ticket corners visibly tucked inside. The box has no crank, no wheel, no mechanical exit, no labels, and no text.
Barkley makes a compact, joyful little hop to the right of the box: one hind paw is lightly lifted from the paper, his head tilts toward the ticket, and his open posture reads as an immediate happy winning moment. Keep his canonical simple face with only the two cream eyes and centred black pupils; the joy comes from the silhouette and pose, not a mouth or extra expression marks. One front paw has just pulled ONE large gold #F8B003 ticket from the top opening. His other front paw holds the ticket's far edge at chest height so its face turns directly toward the viewer. The ticket is the unmistakable focal point: a broad, chunky horizontal paper shape with two simple semicircle notches cut from its side edges. In the exact centre of this ticket, ONE large chunky red #CB2108 five-point star is printed as the winner mark. The star is on the ticket, not floating in the air. Keep a clear cream-paper gap between both arms and Barkley's torso so the two-handed reveal reads at thumbnail size.""",
}
