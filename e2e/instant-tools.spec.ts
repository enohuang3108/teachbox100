...
test("計時器開始後可暫停，重設後回到可開始狀態", async ({ page }) => {
  ...
  // 頂列與右下角各有一顆「重設」，做的事一樣
  await page.getByRole("button", { name: "重設", exact: true }).first().click();
...
test("計分板點組別加分，切換分數步長後依新步長累加", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "一次加減 1 分" }).click();
  await page.getByRole("menuitemradio", { name: "5 分" }).click();
