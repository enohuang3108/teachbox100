...
export default function MonopolyGame() {
  ...
  function runWalk(desc: {
    ...
    const advance = () => {
      ...
      // 老師關掉加碼題時規則已直接發錢，照常走完再播 🏁 過場
      if (
        step === passStep &&
        useMonopolyStore.getState().game?.pendingAction?.kind ===
          "passStartQuestion"
      ) {
  ...
  const center = (
    <div className="flex h-full w-full flex-col items-center justify-center gap-4 px-4 py-2">
      ...
      <Dice
        ...
        diceCount={game.settings.diceCount}
        rolling={rolling}
        rollSeq={rollSeq}
